module.exports = app => {
  ...
  router.get('/heroes/list',async(req,res)=>{
    ...
    cats.unshift({
      name:'热门',
      list:await Hero.find().where({categories:{$in:catIdList}}).populate('categories').limit(10).lean()
    })

    //将每个分类下面的newsList加上categoryName字段，同该分类的name
    cats.map(cat=>{
        cat.list.map(hero=>{
          const categoryName = (cat.name==='热门')? hero.categories[0].name : cat.name
          hero.categoryName = categoryName//如果分类是‘热门’，就取该条新闻分类的第一个分类
          return hero
        })
    })
    res.send(cats)
  ...
  router.get('/hero/:id',async(req,res)=>{
    const hero = await Hero.findById(req.params.id).populate('categories items1 items2 counters.hero canCounterHeroes.hero partners.hero').lean()
    res.send(hero)
  })
